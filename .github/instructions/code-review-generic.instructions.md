...
1. **Give Examples**: Reference similar patterns in the codebase when suggesting changes
1. **Break Complex Tasks**: Review large PRs in logical chunks (security → tests → logic → style)
1. **Avoid Ambiguity**: Be specific about which file, line, and issue you're addressing
1. **Indicate Relevant Code**: Reference related code that might be affected by changes
1. **Experiment and Iterate**: If initial review misses something, review again with focused questions
